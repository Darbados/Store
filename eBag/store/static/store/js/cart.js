$(document).ready(function(){
    ...
    let products_info = check_storage()
    cart_details(products_info, initial_table_html)
    ...
    $(".cart_quantity_delete").on('click', function(e){
        ...
        cart_details(all_orders, initial_table_html)
    })
});


function cart_details(data, base){
    let template;

    for (let x=0; x<data.length; x++){
        if (data[x] != ''){
            product_details = data[x].split('|')
...
function check_storage(){
    ...
    if (window.sessionStorage.hasOwnProperty('Products') && window.sessionStorage.getItem('Products').length > 0){
        all_orders = window.sessionStorage.getItem('Products').split(',')
    }
